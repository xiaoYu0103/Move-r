...
{  
  ...
  if(nn%4!=0) {
    if (log) printf("Invalid input file: size not multiple of 4\n");
    exit(1);
  }
  #if !M64
  // if in 32 bit mode, the number of words is at most 2^31-2
  if(nn/4 > 0x7FFFFFFE) {
    printf("\nInput containing more than 2^31-2 phrases!\n");
  ...
  if(nn/4 > 0xFFFFFFFEu) {
    if (log) printf("Input containing more than 2^32-2 phrases!\n");
    if (log) printf("This is currently a hard limit\n");
    exit(1);
  }
  #endif
  if (log) printf("Parse file contains %ld words\n",nn/4);
  long n = nn/4;
  // ------ allocate and read text file, len is n+1 for the EOS 
  uint32_t *Text = (uint32_t*) malloc((n+1)*sizeof(*Text));
...
static void parseArgs(int argc, char** argv, Args_Bwtparse *arg ) {
  ...
  arg->compute_sa_info = false;
  arg->th = 0;
  while ((c = getopt( argc, argv, "sht:") ) != -1) {
    switch(c) {
      case 's':
      arg->compute_sa_info = true; break;
      case 'h':
...
static sa_index_t *compute_SA(uint32_t *Text, long n, long k, bool log) 
{
  ...
  if (log) printf("Computing SA of size %ld over an alphabet of size %ld\n",n,k);
  int depth = sacak_int(Text, SA, n, k);
  if(depth>=0) {
    if (log) printf("SA computed with depth: %d\n", depth);
  } else {
...
static uint8_t *load_sa_info(const Args_Bwtparse& arg, long n)
{  
...
void bigbwt_bwtparse(const Args_Bwtparse& arg, bool log)
{
  ...
  sa_index_t *SA = compute_SA(Text,n+1,k+1,log);
  ...
  if(arg.compute_sa_info) get_and_write_myint(sa_info,n,n-1,sa_out); // ending position+1 of Text[n-1] in original text T (is |T|+w) 
  ...
  for(long i=1;i<=n;i++) {
    if(SA[i]==0) {  
      ...
      if(arg.compute_sa_info) write_myint(0,sa_out); // dummy end of word position, it is never used an 0 does not appear elsewhere in sa_out
    }
    else {
      if(SA[i]==1) {
      ...
      if(arg.compute_sa_info) get_and_write_myint(sa_info,n,SA[i]-1,sa_out); // ending position of BWT symbol in original text
      BWTsa[i] = Text[SA[i]-1];
    }
  }
  if(fclose(lastout)!=0) die("bwlast close");
  if (log) printf("---- %ld bwlast chars written ----\n",n+1);
  free(last);
  ...
  if (log) puts("---- computing inverted list ----");
  ...
  fclose(ilist);
  if (log) printf("---- %ld ilist positions written (%ld bytes) ----\n",n+1,(n+1)*4l);
  // deallocate
  free(F);
  free(occ);
  free(SA);
  free(Text);
  if (log) printf("==== Elapsed time: %.0lf wall clock seconds\n", difftime(time(NULL),start_wc));  
}
