...
typedef struct {
  ...
  Args_Newscan *arg;       // command line input
...
void *mt_parse(void *dx)
{
  ...
  // prepare for parsing
  arg->input.seekg(d->start); // move to the begining of assigned region
  KR_window krw(arg->w);
  int c; string word = "";
  ...
  if(d->start==0) {
    ...
    while( (c = arg->input.get()) != EOF ) {
      ...
      d->skipped++;
      if(d->true_start + d->skipped == d->true_end + arg->w) {arg->input.close(); return NULL;}
      word.append(1,c);
      uint64_t hash = krw.addchar(c);
      if(hash%arg->p==0 && d->skipped >= arg->w) break;
    }
    if(c==EOF) {arg->input.close(); return NULL;} // reached EOF without finding a breaking point nothing to do
  ...
  while( (c = arg->input.get()) != EOF ) {
    ...
    if(hash%arg->p==0 && d->parsed>arg->w) {
      ...
      d->words++;
      if(d->true_start+d->skipped+d->parsed>=d->true_end+arg->w) {arg->input.close(); return NULL;}
    }
  }
  // end of file reached
...
uint64_t mt_process_file(const Args_Newscan& arg, map<uint64_t,word_stats>& wf, bool log)
{
  Args_Newscan arg_ = arg;
  // get input file size
  size_t size = arg.input.tellg();
  arg.input.close();
...
// prefix free parse of file fnam. w is the window size, p is the modulus
// use a KR-hash as the word ID that is written to the parse file
uint64_t mt_process_file_fasta(Args_Newscan& arg, map<uint64_t,word_stats>& wf, bool log)
{
  ...
  if (log) cout << "total parsed characters: " << tot_char <<  ", file size: " << size << endl;
