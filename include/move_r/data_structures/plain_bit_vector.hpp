...
protected:
    ...
    void setup()
    {
        if constexpr (build_rank_support)
            rank_1_support = sdsl::bit_vector::rank_1_type(&vec);
        if constexpr (build_select_0_support)
            select_0_support = sux::bits::SimpleSelectZero<>(vec.data(), vec.size(), 3);
        if constexpr (build_select_1_support)
            select_1_support = sux::bits::SimpleSelect<>(vec.data(), vec.size(), 3);
