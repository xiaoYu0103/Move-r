...
public:
    ...
    void copy_from_other(const idx_bit_vector& other) {
        samples = other.pred;
        set_rs();
    }

    void move_from_other(idx_bit_vector&& other) {
        other.reset_rs();
    ...
    void set_rs() {
        pred.set_vector(&samples);
        run_sample.set_vector(&samples);
    }

    void reset_rs() {
        pred.set_vector(NULL);
        run_sample.set_vector(NULL);
    }
    ...
    idx_bit_vector(idx_bit_vector&& other) {move_from_other(std::move(other));}
    idx_bit_vector(const idx_bit_vector& other) {copy_from_other(other);}
    idx_bit_vector& operator=(idx_bit_vector&& other) {move_from_other(std::move(other));return *this;}
    idx_bit_vector& operator=(const idx_bit_vector& other) {copy_from_other(other);return *this;}
