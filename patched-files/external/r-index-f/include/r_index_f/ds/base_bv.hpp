...
public:
    ...
    void copy_from_other(const base_bv& other) {
        base = other.base;
        diff_bv = other.diff_bv;
        diff_select = other.diff_select;

        diff_select.set_vector(&diff_bv);
    }

    void move_from_other(base_bv&& other) {
        base = other.base;
        diff_bv = std::move(other.diff_bv);
        diff_select = std::move(other.diff_select);

        other.base = 0;
        other.destroy_select();
    }

    void destroy_select() {
        diff_select.set_vector(NULL);
        diff_select = std::move(bv_select_1());
    }

    base_bv() {}

    base_bv(base_bv&& other) {move_from_other(std::move(other));}
    base_bv(const base_bv& other) {copy_from_other(other);}
    base_bv& operator=(base_bv&& other) {move_from_other(std::move(other));return *this;}
    base_bv& operator=(const base_bv& other) {copy_from_other(other);return *this;}
