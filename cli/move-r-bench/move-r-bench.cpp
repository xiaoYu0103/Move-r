...
void help(std::string msg)
{
    ...
    std::cout << "              has to be executed from the base folder." << std::endl
              << std::endl;
...
void bench_indexes()
{
    ...
    bench_index<pos_t, move_r<_locate_rlzdsa, char, pos_t>, true, true>("move-rlzdsa", "move_rlzdsa");
...
void parse_args(char** argv, int argc, int& ptr)
{
    ...
    if (s == "-m") {
        if (ptr >= argc - 1)
            help("error: missing parameter after -m option");
        path_mf = argv[ptr++];
...
int main_bench_a(int argc, char** argv)
{
    ...
    if (!patterns_file_1.good())
        help("error: invalid input, could read <patterns_file_1>");
    if (!patterns_file_2.good())
        help("error: invalid input, could read <patterns_file_2>");
...
int main_bench_indexes(int argc, char** argv)
{
    ...
    if (!input_file.good())
        help("error: invalid input, could not read <input_file>");
    if (!patterns_file_1.good())
        help("error: invalid input, could not read <patterns_file_1>");
    if (!patterns_file_2.good())
        help("error: invalid input, could not read <patterns_file_2>");
    if (mf.is_open() && check_correctness)
        help("error: cannot output measurement data when checking for correctness");
    ...
    if (check_correctness)
        std::cout << "correctnes will be checked if possible" << std::endl;
    std::cout << std::endl;
...
int main(int argc, char** argv)
{
    ...
    if (std::string(argv[1]) == "-a")
        return main_bench_a(argc, argv);
    else
        return main_bench_indexes(argc, argv);
}